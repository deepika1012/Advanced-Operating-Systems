...
public class FileSystem implements Runnable, ActionListener {
	...
	static int[] portN = new int[45];
	static int[] voteWgt = new int[45];
	static String[] ipAddr = new String[45];

	static int[] lockMembers = new int[45];
	
	static int[] accessMode = new int[50];
	static int[] versionNum = new int[50];
	static int[] sharedCount = new int[50];
	static int[] writeCount = new int[50];
	static int[] readCount = new int[50];
	...
	public synchronized void run() {
		chkInActTimer = new Timer(50000, this);
		...
		try {
			...
			while (true) {
				...
				if(WRITE == Integer.parseInt(tokens[0])) {
					System.out.println("* FS: DESTINATION WRITE: " + "file" + Integer.parseInt(tokens[1]) + Application.procId + ", Info: " + tokens[2]);
				...
				try {
					...
					// e.printStackTrace();
					System.out.println("* FS: Reply Failed for a " + modes[Integer.parseInt(tokens[0])] +" Request");
					
					if (EXCLUSIVE_MODE == Integer.parseInt(tokens[0])) {
						if(!rplyData.equals(Integer.toString(FALSE))){
							accessMode[Integer.parseInt(tokens[1]) - 1] = 0;
							writeCount[Integer.parseInt(tokens[1]) - 1]--;
						}
					} else if (SHARED_MODE == Integer.parseInt(tokens[0])) {
...
//		for (int i = 1; i <= Application.MaxFiles; i++) 
//			System.out.println("* File"+ i + "Version Num: " + versionNum[i-1]);
//		
//		for (int i = 1; i <= Application.MaxFiles; i++) 
//			readFile(i);
	...
	public int acquireLock(int mode, int fileNo) {
		...
		for (int i = 0; i < Application.totalNodes; i++) {
			...
			try {
				...
				{
					System.out.println("* FS: Site " + ipAddr[i] + "'s vote is Unavailable");
				}
...
//		for (int i = 0; i < Application.MaxFiles; i++) 
//		{
//			fname = "file" + (i + 1) + Application.procId;
//			try {
...
//					tokens = line.split(":");
//
//				versionNum[i] = Integer.parseInt(tokens[1]);
//
//				br.close();
//			} catch (Exception e) {
//				// e.printStackTrace();
...
//		}
//		for (int i = 0; i < Application.MaxFiles; i++) 
//			System.out.println("* FS: File" + (i+1) + ": " + versionNum[i]);
	}
...
//			System.out.println("* FS: Connecting to: "  + ipAddr[lockMembers[i] - 1] + ", "  + portN[lockMembers[i] - 1]);
	...
	public void writeFile(String data, int fileNo) {
		...
		try {
			...
			fileout.flush();
			fileout.close();
//			System.out.println("* FS: WRIT " + fname + ": Version Number: " + divide[0] + ", Data: " + divide[1]);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	protected int writeFileAndUpdateOthers(int fileNo) {
		...
		System.out.println("* FS: SOURCE WRITE: " + "file" + fileNo + Application.procId + ", Info: " + writeData);
...
//			System.out.println("* FS: My version number is the latest: " + versionNum[fileNo-1] + ", Max Version: " + maxVersion);
...
//				System.out.println("* FS: Reply Buffer Contents are: " + rplyData);
				...
				System.out.println("* FS: READ UPDATE WRITE: " + "file" + fileNo + Application.procId + ", Info: " + rplyData);
