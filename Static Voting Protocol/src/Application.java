	...
	public static synchronized void main(String[] args) {
		...
		try {
			...
			while ((currLine = br.readLine()) != null) 
			{
							...
							try {
								...
								while(0 > status && retryCount < MAX_RETRY)
								{
									...
									if(CHECK_OUT_FAIL == status) 
									{
										retryCount++;
										System.out.println("* ----------------");
										System.out.println("* Application: Retry Attempt: " + retryCount + " after " + backOffTime + " secs");
				...
				if(status < 0)
				{
					if(WRITE_FAIL == status || READ_FAIL == status)
					{
						try {
							localFS.writeLog(tokens[1] + " Failed", Integer.parseInt(tokens[2]));
							localFS.writeLog("]", Integer.parseInt(tokens[2]));
							System.out.println("* Application: Failed Delay for " + Integer.parseInt(tokens[2]) + " seconds");
							Thread.sleep(Integer.parseInt(tokens[2]) * 1000);
						} catch (InterruptedException e) {
							e.printStackTrace();
						}
				...
				if(tokens[3].equals("T")) 
				{
					try {
					        ...
					        long rand1 = (long) (rn.nextGaussian()*4 + 10) * 1000;
					        if(rand1 < 0) rand1 *= -1;
							System.out.println("* Application: --------------------- SYSTEM FAILURE: " +rand1+ " -------------------------");
		...
		System.out.println("* -------------------------------------------------------");
		System.out.println("* Application " + Application.procId + ": Return Status: Waiting for Thread");
